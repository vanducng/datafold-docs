...
Open Source Data Diff splits the table into smaller segments, then checksums each segment in both databases. When the checksums for a segment aren't equal, it will further divide that segment into yet smaller segments, checksumming those until it gets to the differing row(s).
...
**†:** The implementation for downloading all rows that Open Source Data Diff and
...
Open Source Data Diff can diff tables within the same database, or across different databases.
...
In this section we'll be doing a walk-through of exactly how Open Source Data Diff
...
In order to check whether the two tables are the same, Open Source Data Diff splits
...
Open Source Data Diff starts by querying both databases for the `min(id)` and `max(id)`
...
Now Open Source Data Diff will start running `--threads=1` queries in parallel that
...
`--interactive`. This puts Open Source Data Diff in interactive mode, where it shows an
...
Now Open Source Data Diff will do exactly as it just did for the _whole table_ for only
...
and compare them in memory in Open Source Data Diff.
...
Finally Open Source Data Diff will output the `(id, updated_at)` for each row that was different:
...
* If there are very large gaps in your key column (e.g., 10s of millions of
  continuous rows missing), then Open Source Data Diff may perform poorly, doing lots of
...
* The fewer columns you verify (passed with `--columns`), the faster
  Open Source Data Diff will be. On one extreme, you can verify every column; on the
...
* We have ideas for making Open Source Data Diff even faster that
