...
## How does Open Source Data Diff work?

Open Source Data Diff compares _every data value_ that is generated by your dev branch code and compares those values with the data in prod.

## Why is Open Source Data Diff important?

Open Source Data Diff catches unexpected data changes that can't be caught by dbt tests.
...
**There's no dbt test that could catch this.** But Open Source Data Diff can.

Open Source Data Diff alerts you to data changes that:
...
### Install Open Source Data Diff.
We recommend installing Open Source Data Diff in the same virtual environment that you use when running dbt CLI.

```
pip install data-diff
```

Then, install a driver specific to the database you want to connect to. We'll assume you're using Snowflake, but Open Source Data Diff supports additional databases as well.

```
pip install 'data-diff[snowflake]'
```

### Run Open Source Data Diff to compare dev and prod data.

To run Open Source Data Diff from the command line and compare the data in your prod schema to your dev schema, run this command:
...
- Each line beginning with a hyphen represents an _option_. In the command above, we've included the main options you will likely need:
  ...
  - `-m` specifies a prefix that will be used to write the results of your Open Source Data Diff run to a Diff Results Table into your warehouse.
  - `--materialize_all_rows` instructs Open Source Data Diff to write _all_ rows to the Diff Results Table, not only conflicts. This is helpful if you want to query the results and calculate statistics such as the percentage of rows that have a conflicting value in a given column.
...
It's far more powerful to _compare every value_ with Open Source Data Diff instead of writing ad hoc SQL statements to count rows and generate summaries. And you don't have to write a single join.

**Structure of the Diff Results Table**

We'll assume you're investigating the primary key, `org_id`, and the `status` column, because the dev code contains edits to the `status` column. We've also included the `created_at` column to support our analysis of the results.
...
Writing SQL to get that high value information is easy! It's also extensible if you want to write complex analysis and joins. That's the value you get out of using Open Source Data Diff instead of doing this manually.
...
<img width="73" alt="Screen Shot 2022-11-16 at 10 17 22 AM" src="https://user-images.githubusercontent.com/1799931/202260999-c12f43c4-31fb-4968-9b9d-92ab897a3bbf.png"></img>
...
By materializing the results of Open Source Data Diff in the warehouse, we gained insight into data changes on the level of individual values, as well as the ability to quickly summarize the Diff Results Table into meaningful conclusions about the impact the dev code will have on the data.

Once we've completed this analysis, we can make any changes needed to the code, run Open Source Data Diff again, and finally, ask for a codereview from your teammate.

#### Here's the best part.

When you ask for a code review, you can say: "Hi, can you please review this PR? I've already run Open Source Data Diff, so you don't need to worry about any unexpected impacts on the data."
